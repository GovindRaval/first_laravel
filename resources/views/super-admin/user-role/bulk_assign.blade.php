                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="select2 select-user {{ $errors->has('user_id') ?'error':'' }}" name="user_id[]" multiple="">
                                        @foreach  ($userList as $record)
                                        <option value="{{$record->id}}" {{ (collect(old('user_id'))->contains($record->id)) ? 'selected':'' }}>{{$record ? ucfirst($record->name). ' ' .ucfirst($record->last_name) .'('.$record->email.')' : ''}}</option>
                                        @endforeach
                                    </select>
                                    @error('user_id')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="select2 select-role {{ $errors->has('role_id') ?'error':'' }}" name="role_id[]" multiple="">
                                        ...
                                        <option value="{{$record->id}}" {{ (collect(old('role_id'))->contains($record->id)) ? 'selected':'' }}>{{$record ? ucfirst($record->name) :''}}</option>
                                        @empty
                                        <option></option>
                                        @endforelse
                                    </select>
