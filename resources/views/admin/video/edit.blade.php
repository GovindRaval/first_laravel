    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card {{config('custom.card-primary')}}">
                ...
                <form method="post" action="{{route('admin.video.update',['id' => $singleRecord->id])}}">
                    @csrf
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="col-md-12 col-sm-12 p-0">
                                    <div class="form-group">
                                        <label>{{__('admin.video')}} ({{ucfirst($language->name)}})</label><span class="required"></span>
                                        <input name="video_{{$language->id}}" type="text" class="form-control length-30 {{ $errors->has('video_'.$language->id) ?'is-invalid':'' }}" placeholder="{{__('admin.video')}} ({{ucfirst($language->name)}})" value="{{old('video_'.$language->id,$record?$record->video_name:'')}}">
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="col-md-12 col-sm-12 p-0">
                                    <div class="form-group">
                                        <label>{{__('admin.video-url')}}</label><span class="required"></span>
                                        <input name="video_url" type="text" class="form-control length-30" placeholder="{{__('admin.video-url')}}({{$language->name}})" value="{{old('video_'.$language->id,$record?$record->video_url:'')}}">
                                        @error('video_url'.$language->id)
                                        <div class="text-red">{{ $message }}</div>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-md-12 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="sorting" placeholder="{{__('admin.sorting_order')}}" class="form-control number-input" value="{{old('sorting',$singleRecord->sorting!=0?$singleRecord->sorting:$sortingNumber)}}">
                                    @error('sorting')
                                    <div class="text-red">{!! $message !!}</div>
                                    @endif
                                </div>
