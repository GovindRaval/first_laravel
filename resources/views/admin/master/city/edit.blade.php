    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card {{config('custom.card-primary')}}">
                ...
                <form method="post" action="{{route('admin.master.city.update',['id' => $singleRecord->id])}}">
                    @csrf
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-4 col-sm-12">
                                <div class="form-group">
                                    <label>{{__('admin.country')}}<span class="required">*</span></label>
                                    <select class="form-control select2 select-is-feature {{config('custom.select2-css')}}" data-dropdown-css-class="{{config('custom.select2-css')}}" name="country_id">
                                        ...
                                        @endphp
                                        <option value="{{$val->id}}" {{ (collect(old('country_id',$singleRecord->country_id))->contains($val->id)) ? 'selected':'' }}>{{ucfirst($description->country_name)}}</option>
                                        @endforeach
                                    </select>
                                    @error('country_id')
                            ...
                            <div class="col-md-4 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input name="city_{{$language->id}}" type="text" class="form-control length-30 {{ $errors->has('city_'.$language->id) ?'is-invalid':'' }}" placeholder="{{__('admin.city')}} ({{ucfirst($language->name)}})" value="{{old('city_'.$language->id,$record?$record->city_name:'')}}">
