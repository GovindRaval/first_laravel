    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card {{config('custom.card-primary')}}">
                ...
                <form method="post" action="{{route('admin.general-setting.setting.update',['id'=>$record->id])}}" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="setting_val" class="form-control textarea {{ $errors->has('setting_val') ?'is-invalid':'' }}" placeholder="{{$record->setting_key}}">{{old('setting_val',$record->setting_val)}}</textarea>
                                    @elseif($record->type=='radio')
                                    <div class="form-group clearfix">
                                        @foreach($radioValue as $key=>$radio)
                                        <div class="icheck-primary d-inline">
                                            <input type="radio" id="{{$radio}}" value="{{$key}}" name="setting_val" {{ (collect(old('setting_val',$record->setting_val,$record->setting_val))->contains($key)) ? 'checked':'' }}>
                                            <label for="{{$radio}}">{{ucfirst($radio)}}</label>
                                        </div>
                                        @endforeach
                                    </div>
                                    @else
                                    <input name="setting_val" type="text" class="form-control {{ $errors->has('setting_val') ?'is-invalid':'' }}" placeholder="{{$record->setting_key}}" value="{{old('setting_val',$record->setting_val)}}">
                                    @endif
                                    @error('setting_val')
                                    <div class="text-red">{{ $message }}</div>
                                    @endif
